...
import {
  FALLBACK_MAKES,
  FALLBACK_MODELS,
  PREDEFINED_CAR_MAKES,
  PREDEFINED_CAR_MODELS,
} from '@/modules/cars/constants/car-data';
...
export class MakeAndModelSeederService implements OnModuleInit {
  ...
  constructor(
    ...
    this.apiaryApiUrl = this.configService.get<string>(
      'external.external_apiary_url',
    );
  }
  ...
  public async seedMakesAndModelsIfEmpty(): Promise<void> {
    ...
    const cachedMakes = await this.redisService
      .getClient()
      .get(CacheKeys.MAKES);
    const cachedModels = await this.redisService
      .getClient()
      .get(CacheKeys.MODELS);
    ...
    if (this.apiaryApiUrl === undefined) {
      this.logger.warn(
        'Received wrong data from API for makes and models validation',
      );
      useFallbackData();
      return;
    }
    ...
    try {
      ...
      const uniqueMakes = new Set(
        makesToCache.map((make) => make.toLowerCase()),
      );
      makesToCache = Array.from(uniqueMakes).concat(PREDEFINED_CAR_MAKES);

      const uniqueModels = new Set(
        modelToCache.map((model) => model.toLowerCase()),
      );
      modelToCache = Array.from(uniqueModels).concat(PREDEFINED_CAR_MODELS);

      this.logger.log(
        `Successfully fetched car makes and models from external API.`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to fetch car makes from API: ${error.message}.`,
      );

      // Use hardcoded list on failure
      useFallbackData();
      this.logger.warn(
        `Using ${makesToCache.length} hardcoded fallback car makes .`,
      );
    }
    ...
    await this.redisService
      .getClient()
      .set(CacheKeys.MAKES, JSON.stringify(makesToCache), 'EX', 86400);
    await this.redisService
      .getClient()
      .set(CacheKeys.MODELS, JSON.stringify(modelToCache), 'EX', 86400);
